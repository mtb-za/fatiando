...
def upcontinue(x, y, data, shape, height):
    ...
    .. note:: It is not possible to get the FFT of a masked grid. The default `gridder.interp()` call using minimum curvature will not be suitable. Use `extrapolate=True` or `algorithm='nearest'`.
